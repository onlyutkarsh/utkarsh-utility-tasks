...
taskids.tasks.forEach(task => {
    ...
    files.forEach(file => {
        ...
        console.log(`Changed task id from ${chalk.magenta(tempTaskId)} to ${chalk.blue(taskJson.id)}`);
        console.log(`Changed friendlyName from ${chalk.magenta(tempFriendlyName)} to ${chalk.blue(taskJson.friendlyName)}`);
