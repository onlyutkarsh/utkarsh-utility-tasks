...
async function main() {
    try {
        ...
        let rmClient =
            new azrm.ResourceManagementClient.ResourceManagementClient(
                credentials,
                subscriptionId
            );
        ...
        console.info(
            `Getting information about resource group '${resourceGroupName}'`
        );
        let resourceGroup = await rmClient.resourceGroups.get(
            resourceGroupName
        );
        ...
        switch (action) {
            case "Append": {
                console.info(
                    `Appending tags for resource group '${resourceGroup.name}'`
                );
            ...
            case "Replace": {
                console.info(
                    `Replacing tags for resource group '${resourceGroup.name}'`
                );
            ...
            case "Clear": {
                console.info(
                    `Clearing tags for resource group '${resourceGroup.name}'`
                );
...
async function appendTags(
    newTags: any,
    resourceGroup: azrm.ResourceManagementClient.ResourceManagementModels.ResourceGroup,
    rmClient: azrm.ResourceManagementClient.ResourceManagementClient
) {
