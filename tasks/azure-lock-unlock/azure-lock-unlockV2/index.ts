...
async function main() {
    try {
        ...
        let client = new azrm.ManagementLockClient.ManagementLockClient(
            credentials,
            subscriptionId
        );
        ...
        if (action === "Apply") {
            if (applyTo === "ResourceGroup") {
                console.info(
                    `Applying '${lockType}' lock to resource group '${resourceGroupName}'`
                );
                await client.managementLocks.createOrUpdateAtResourceGroupLevel(
                    resourceGroupName,
                    lockName,
                    <
                        azrm.ManagementLockClient.ManagementLockModels.ManagementLockObject
                    >{
                        level: lockType,
                        notes: notes,
                    }
                ...
                await client.managementLocks.createOrUpdateAtSubscriptionLevel(
                    lockName,
                    {
                        level: lockType,
                        notes: notes,
            ...
            if (applyTo === "ResourceGroup") {
                console.info(
                    `Removing '${lockType}' lock from resource group '${resourceGroupName}'`
                );
                await client.managementLocks.deleteAtResourceGroupLevel(
                    resourceGroupName,
                    lockName
                );
                console.info("Success");
                ...
                await client.managementLocks.deleteAtSubscriptionLevel(
                    lockName
                );
                console.info("Success");
            }
